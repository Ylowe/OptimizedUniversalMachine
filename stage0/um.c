...
Memory_T UM_boot(char *path)
{
        ...
        for (i = 0; i < pgm_length; i++) {
                for (lsb = 32 - BYTE_WIDTH; lsb >= 0; lsb -= BYTE_WIDTH) {
                        byte = getc(program);
                        instr = Bitpack_newu(instr, BYTE_WIDTH, lsb, byte);
                }
...
uint32_t UM_execute(Memory_T mem, UM_instruction instr, uint32_t pc)
{
        ...
        Um_opcode opcode;
        uint32_t value;
        unsigned ra_lsb, rb_lsb, rc_lsb, val_lsb;
        unsigned opcode_lsb = 28;
        unsigned register_width = 3;
        unsigned opcode_width = 4;
        ...
        if (opcode == LV) {
                ...
                value = Bitpack_getu(instr, val_width, val_lsb);
                ra = Bitpack_getu(instr, register_width, ra_lsb);
        ...
        ra = Bitpack_getu(instr, register_width, ra_lsb);
        rb = Bitpack_getu(instr, register_width, rb_lsb);
        rc = Bitpack_getu(instr, register_width, rc_lsb);
