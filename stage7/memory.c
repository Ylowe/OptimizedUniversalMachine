...
bool test_add_and_remove_segment()
{
	...
	if (id != 0)
		return FAIL;
	if (((Segment *)Seq_get(mem->memory, id))->size != 2048)
		return FAIL;
	...
	if (id != 1)
		return FAIL;
	if (((Segment *)Seq_get(mem->memory, id))->size != 1024)
		return FAIL;
...
bool test_at()
{
	...
	Memory_put(mem, id, index, value);

	new_val = Memory_get(mem, id, index);
	if (value != new_val)
		return FAIL;
...
bool test_load_program()
{
	...
	Memory_put(mem, id1, 1, 41);
	Memory_put(mem, id1, 3, 401);
	Memory_put(mem, id1, 16, 583928);

	for (int i = 0; i < 10; i++)
		printf("%d ", Memory_get(mem, 0, i));
	printf("\n");
	for (int i = 0; i < 20; i++)
		printf("%d ", Memory_get(mem, id1, i));
	printf("\n");
	...
	for (i = 0; i < 20; i++) {
		if (Memory_get(mem, id0, i) != Memory_get(mem, id1, i))
			return FAIL;
	}
	...
	Memory_put(mem, id2, 2, 11);
	Memory_put(mem, id2, 7, 4021);
	Memory_put(mem, id2, 15, 5828);
...
void Memory_free(Memory_T *mem) {
	...
	for (i = 0; i < length; i++) {
		...
		Segment *seg = (Segment *)Seq_get((*mem)->memory, i);
		free(seg);
	}
...
int Memory_add_segment(Memory_T mem, int seg_length)
{
	...
	int i;
	int id;
	Segment *seg = malloc(sizeof(*seg) + sizeof(uint32_t) * seg_length);
	seg->size = seg_length;
...
void Memory_remove_segment(Memory_T mem, int id)
{
	...
	Segment *seg = (Segment *)Seq_get(mem->memory, id);
	// UArray_free(&seg);
	free(seg);
...
*/
void Memory_put(Memory_T mem, int id, int index, uint32_t val)
{
	Segment *seg = (Segment *)Seq_get(mem->memory, id);
	seg->data[index] = val;
}

uint32_t Memory_get(Memory_T mem, int id, int index)
{
	Segment *seg = (Segment *)Seq_get(mem->memory, id);
...
{
	Segment *seg0 = (Segment *)Seq_get(mem->memory, 0);
	Segment *program = (Segment *)Seq_get(mem->memory, id);
	int prog_len = program->size;
	int i;

	free(seg0);
	// Seq_put(mem->memory, 0, UArray_new(prog_len, sizeof(uint32_t)));
	Segment *seg = malloc(sizeof(*seg) + sizeof(uint32_t) * prog_len);
	Seq_put(mem->memory, 0, seg);
